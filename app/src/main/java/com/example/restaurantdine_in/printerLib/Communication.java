...
class SendCommandsMultiplePagesThread extends Thread {
    ...
    private static void resultCallback(final Communication.Result result, final int code, final Communication.SendMultiplePagesCallback callback) {
        if (callback != null) {
            Handler handler = new Handler(Looper.getMainLooper());
            handler.post(new Runnable() {
                @Override
                public void run() {
                    callback.onAllFinish(new Communication.CommunicationResult(result, code));
                }
            });
        }
    }
...
class GetSerialNumberThread extends Thread {
    ...
    private static void resultSendCallback(final Communication.Result result, final int code, final String serialNumber, final Communication.SerialNumberCallback callback) {
        if (callback != null) {
            Handler handler = new Handler(Looper.getMainLooper());
            handler.post(new Runnable() {
                @Override
                public void run() {
                    callback.onSerialNumber(new Communication.CommunicationResult(result, code), serialNumber);
                }
            });
        }
    }
